...
const tools = [
  {
    ...
    deps: '<script src="../js/engine.min.js" defer></script><script src=\"https://unpkg.com/wanakana\"></script>',
  ...
  },
  {
    ...
    deps: '<script src="https://unpkg.com/wanakana" defer></script>',
  ...
  },
  {
    ...
    logic: "if(!window.kuroshiroInstance){renderPhonemes('Please wait, loading Kanji dictionary (20MB)...');return;}renderPhonemes('Converting...');window.kuroshiroInstance.convert(val,{to:'katakana'}).then(function(result){outEl.textContent=result;renderPhonemes('Powered by Kuroshiro & Kuromoji');}).catch(function(err){console.error(err);renderPhonemes('Error parsing Kanji');});",
  ...
  },
  {
    ...
    logic: "if(!window.kuroshiroInstance){renderPhonemes('Please wait, loading Kanji dictionary (20MB)...');return;}renderPhonemes('Converting...');window.kuroshiroInstance.convert(val,{to:'hiragana'}).then(function(result){outEl.textContent=result;renderPhonemes('Powered by Kuroshiro & Kuromoji'); if(sourceTag) sourceTag.textContent='Japanese reading parser';}).catch(function(err){console.error(err);renderPhonemes('Error parsing Kanji');});",
    deps: '<script src="https://unpkg.com/kuroshiro@1.2.0/dist/kuroshiro.min.js" defer></script><script src="https://unpkg.com/kuroshiro-analyzer-kuromoji@1.1.0/dist/kuroshiro-analyzer-kuromoji.min.js" defer></script><script>window.kuroshiroInstance=null;document.addEventListener("DOMContentLoaded",function(){var k=new Kuroshiro();k.init(new KuromojiAnalyzer({dictPath:"https://unpkg.com/kuromoji@0.1.2/dict"})).then(function(){window.kuroshiroInstance=k;console.log("Kuroshiro ready");});});</script>',
    emptyOutput: 'ひらがな',
  ...
  },
  {
    ...
    logic: "var out = ToolConverters.convertChineseToKatakana(val); outEl.textContent = out.katakana || 'カタカナ'; renderPhonemes(out.phonemes); if(sourceTag) sourceTag.textContent = 'Chinese pinyin conversion';",
    deps: '<script src="https://unpkg.com/pinyin-pro@3.27.0/dist/index.umd.js" defer></script>',
...
tools.forEach(function (tool) {
  ...
  locHtml = ensureReplaced(
    locHtml,
    /<label for="english-input" class="row-label">.*?<\/label>/,
    '<label for="english-input" class="row-label">' + tool.inputLabel + '</label>',
    'input label'
  );
  locHtml = ensureReplaced(
    locHtml,
  ...
  locHtml = locHtml.replace('<script src="https://unpkg.com/wanakana" defer></script>', '');
  ...
  const helperScript = tool.isFile ? '<script src="js/tool-converters.min.js" defer></script>' : '<script src="../js/tool-converters.min.js" defer></script>';
